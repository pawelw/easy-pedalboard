...
3. Build the missing components against the light theme only:
   ...
   `StageGroup`/`StageControl`, `StageRouter`, `LinkBrace`, `UnitSwitch`,
   `Pill`, `LinkIcon`,
   `TapeIcon`, `ModIcon`,
...
1. `plugins/peak-delay/jsui/` — copy the structure of
   ...
   `plugins/*/jsui`).
2. C++: add the four new parameters from the README table (`flutter`,
   `phaser`, `tapepre`, `modpost`) — the two routing parameters mean
   restructuring `processBlock` so each stage can sit either side of the delay
   line; scope that first and split it out if it is too large for this pass.
   Then a `PeakDelayWebEditor` mirroring `PeakWahWebEditor` — slider relays for
   `mix`, `fb`, `ltime`, `rtime`, `mod`, `tape`, `flutter`, `phaser`,
   and toggle/choice relays for `tapepre`, `modpost`; toggle relays for `sync`,
