# BitBit Alpine — implementation plan

A twelfth plugin, `plugins/bitbit-alpine`: one host panel wrapping three effect
modules — **Modulation** (Tape / Tremolo / Chorus / Phaser), **Delay** (the
existing BitBit Delay face and chain, embedded), **Reverb** (Space / Spring).

Design handover: `design_handoff_peak_alpine/README.md`, prototype at
`design_handoff_peak_alpine/BitBit Alpine.dc.html` (serve it — the file
...
gallery at `#bitbit-alpine`, bound through `ParamScope` to the namespaced
parameters the processor will carry. `ee::dsp::Tremolo` and `ee::fx::DelayModule`
are both extracted, with BitBit Trem & Pan and BitBit Delay running on them and
rendering sample-exact, and `SpringReverb` has its two new controls with BitBit
Spring untouched, and both side modules exist with their engine crossfade
proven click-free. **`plugins/bitbit-alpine` builds and makes sound** - 46
parameters, all three modules in the chain, driven by `ee_alpine_host`, and the
real face is bound to them through `BitBitAlpineWebEditor`. Stage 10 (the factory
preset bank) not started.

None of the eleven existing pedals is replaced. BitBit Delay in particular keeps
shipping exactly as it does today; BitBit Alpine holds a second instance of the
...
1. **The DSP is scattered across six processors.** Four of the seven engines
   ...
   ~250 lines inline in `BitBitTremPanProcessor::processBlock`. Neither does the
   *delay chain*: `ee::dsp::TapeDelay` is only the delay line, and the tape
   ...
   BitBit Delay sound like BitBit Delay all live in `BitBitDelayProcessor`.
2. **The face is new chrome around a face that already exists.** The Delay
   module is `plugins/bitbit-delay/jsui/src/App.jsx` minus its Card; everything
...
`bitbit-alpine` be thin at both ends.
...
| Tempo divisions | `ee::dsp::TempoDivision`, `plugins/bitbit-delay/src/TimeMap.h` |
...
| WebView plumbing | `ee::plugin::webface`, `BitBitDelayWebEditor` as the template |
| Plugin boilerplate | `bitbit_add_plugin(... WEBVIEW)` |
...
Lifted verbatim from `BitBitTremPanProcessor::processBlock`: the shaped LFO
...
`BitBitTremPanProcessor` then becomes a parameter → engine adapter.
...
The whole of BitBit Delay's chain: `TapeDelay`, the two `TapeSection`s and their
...
`BitBitDelayProcessor` keeps: its parameter layout, `timeReadout`/`timeMsReadout`/
...
- `ee_preset_tests` must stay green — it runs against `BitBitDelayProcessor` and
...
bit-identical to the current one and **BitBit Spring is untouched**. Only BitBit
Alpine drives them.
...
|---|---|---|
| `ModulationModule` | `TapeMachine`, `Tremolo`, `Chorus`, `Phaser` | The Tape engine is now the whole of BitBit Tape: Tone (a bipolar knob on its own third row) and a Mono/Stereo switch (pinned to the foot of the body) are both on the face, and the recorded tape floor is handed to it so Noise plays the recording rather than the synth-hiss fallback. Tape has **no Mix** — the transport's wow makes its wet path wander, so any partial blend against the dry combs and is heard as tremolo; it runs fully wet, the footer strip stays but the knob is dropped (`MultiEngineModule::engineUsesMix`). Tremolo's Panning stays at its default; Tremolo's rate keeps the free-running Hz mapping (tempo sync stays BitBit Trem-Pan's). |
| `ReverbModule` | `FdnReverb`, `SpringReverb` | Both are mono-in/stereo-out; the module sums to mono for the send exactly as `BitBitReverbProcessor` and `BitBitSpringProcessor` do. |

---

## 4. The plugin — `plugins/bitbit-alpine`

```
plugins/bitbit-alpine/
  CMakeLists.txt              bitbit_add_plugin(BitBitAlpine CODE Palp ... WEBVIEW)
  presets/                    factory bank, categorised by " - " prefix
  src/PluginProcessor.{h,cpp}
  src/BitBitAlpineWebEditor.{h,cpp}
...
**Delay (15):** `dly.on`, then the leaf names BitBit Delay already uses —
...
Leaf names kept identical on purpose: `<DelayFace prefix="dly.">` and BitBit
...
Holds three modules, one APVTS, one `PresetStore { apvts, "BitBit Alpine",
EE_FACTORY_PRESETS }`, the two trims, the global engage crossfade, and the
playhead BPM cache (`readPlayHeadBpm` lifted from BitBit Delay — audio thread
...
`BitBitAlpineWebEditor` is `BitBitDelayWebEditor` with more relays. Same
...
| `@synthpeak/pedal-ui/juce` | a subpath export holding the generic JUCE bindings currently in `bitbit-delay/jsui/src/juceBindings.jsx`: `useJuceSliderValue`, `useJuceToggleValue`, `useJuceChoiceValue`, `JuceKnob`, `JuceFader`, `JucePill`, `JuceChoicePill`, `JuceStageKnob`, `JuceStageRouter`. A subpath, so `pedal-ui`'s main entry stays JUCE-free for the gallery. Takes an optional parameter-id prefix from context. |

Changed:

| Component | Change |
|---|---|
| `PresetBar` | `variant="separated"`: four discrete 8px-radius controls (28×28 prev, 28×28 next, 190×28 name, gap 6; then 28×28 save at gap 10) instead of today's joined segmented group. The joined look stays the default — BitBit Delay and Wah keep it. |
| `Card` | one new prop, `subtitlePlacement="below"`, for the stacked title/tagline column. Everything else about the host panel (padding, 32px logo, 19px title, zero body padding) is scoped in the face's own CSS as `.pa-card`, exactly the way `.pd-card` already scopes BitBit Delay's. |
...
`useHostBpm`) and the layout CSS currently in `bitbit-delay/jsui/src/index.css`.

- `plugins/bitbit-delay/jsui/src/App.jsx` becomes `<Card …><DelayFace/></Card>` —
  a wrapper of about fifteen lines. **Its rendered output must not change.**
- `plugins/bitbit-alpine/jsui` renders `<DelayFace prefix="dly." />` inside a
  `ModulePanel`.
- Verified by a **geometry fingerprint** rather than by eye: with the gallery on
  `#bitbit-delay`, walk every rendered descendant of `.pui-card` and hash
...
### 5.3 `plugins/bitbit-alpine/jsui`
...
- `apps/pedal-gallery/src/pedals.js` gains `{ slug: "bitbit-alpine", face:
  BitBitAlpineFace, theme: "onyx" }`.
...
| 2 | ✅ `packages/delay-face`; BitBit Delay's App.jsx reduced to a wrapper; the generic JUCE bindings to `@synthpeak/pedal-ui/juce` | fingerprint identical on **both** faces — Delay `324 / 626x481 / 733c7d76 / 42296266`, Wah `374 / 566x469 / cf04e593 / d93913e7` (Wah moved too: one `installAutoResize`) |
| 3 | ✅ `plugins/bitbit-alpine/jsui`: the whole face | gallery `#bitbit-alpine` vs the prototype at :3200; card 1046 wide and tracks 186/598/186 exact, every module-shell measurement matches, and the shim's "unknown to the backend" warnings confirm the full namespaced id set |
| 4 | ✅ `ee::dsp::Tremolo` extracted; BitBit Trem-Pan delegates | `ee_trempan_regress`: 13 passes, all checksums identical either side of the change. `ee_trempan_stress`: 7776 cases, 0 flagged |
| 5 | ✅ `ee::fx::DelayModule` extracted; BitBit Delay delegates | `ee_delay_regress`: 14 passes, all checksums identical either side. `ee_preset_tests`: identical. `ee_trempan_regress` unaffected |
| 6 | ✅ `SpringReverb`'s two new setters (Tension, Low Cut) | `ee_spring_regress`: BitBit Spring identical, and the engine section proves the defaults inert, the controls live, and both extremes stable. `ee_reverb_stress` 1512 cases 0 flagged; `ee_dsp_tests` at its known baseline |
...
| 8 | ✅ `plugins/bitbit-alpine`: processor, 46-parameter layout, CMake | `ee_alpine_host`: makes sound, all three power toggles reach the audio, bypass bit-exact, all 8 engine pairs finite and audible |
| 9 | ✅ `BitBitAlpineWebEditor` + `ee::plugin::RelaySet`; face bound to real parameters | Standalone launches, editor constructs, WebView loads, `ee_alpine_host` still green. `dev` build installs AU + VST3 universal (`x86_64 arm64`); **`auval -v aufx Balp BtBt` → AU VALIDATION SUCCEEDED**. Still not done: Ableton Live — see §7 |
| 10 | Factory preset bank | `ee_preset_tests` extended to BitBit Alpine's bank (every parameter in every file) |
...
README and `BitBit Alpine.dc.html` carry them, so the bundle stays the record.
...
2 and 3 land on BitBit Delay as well as BitBit Alpine: one Delay face, no variant,
...
whatever panel is behind them, which was the band and is now the card on BitBit
Delay and the module on BitBit Alpine. It reads `var(--pui-stage-ground,
...
look if a light BitBit Alpine is ever a real face.
...
The README carries them; `BitBit Alpine.dc.html` still shows the pre-tuning look.
...
2. **`SOFT_SWEEP_WIDTH_SMALL` 3 → 1.5.** Below 60px the arc is a hairline; at
   ...
   from it and drops 6.5px → 5px, which tightens BitBit Delay's stage rows by
   1.5px per gap - `StageControl.css` and `StageGroup.css` both spend it.
3. **`.pui-caption` 11px → 10px.** Shared by `Knob`, `Slider`, `Toggle` and
   `Dropdown`, so this is every pedal's captions, not BitBit Alpine's.
...
- **The face is built on the real JUCE bindings, not local state.** The plan
  ...
  BitBit Delay has always rendered in the gallery - so the face is the real one
  from the start.
...
- **The prototype's Delay module is ~9px taller than the real face.** Its row
  measures 150 and its footer 125; BitBit Delay's own face measures 138 and 128,
  and BitBit Alpine renders exactly those. The handoff says to embed the real
...
house rule). BitBit Trem & Pan keeps its parameters, its Rate map, the playhead
...
`BitBitDelayProcessor` went 883 lines to 554 and its header 373 to 224. What left
...
- **The module takes real units only** - seconds, hertz, linear gain. Parameter
  ...
  whoever owns the parameters, which is what lets BitBit Alpine bind the same
...
`BitBitDelayProcessor::processBlock` is one chunk loop doing, in order: the
...
- **Low Cut moves `kOutputLowCutHz`**, the pickup's high-pass on the wet output.
  ...
  dies. Given `FdnReverb`'s own 20-800 Hz range deliberately: BitBit Alpine puts a
...
- **The harness had to grow a second kind of section.** For a *move*, rendering
  ...
  never setting them - which is the claim "BitBit Spring is untouched" actually
  rests on.
...
`plugins/bitbit-alpine` exists: 46 parameters, the three modules in a fixed
...
- **`ee::plugin::InputMeter`** - the level follower and note-onset detector the
  ...
  design, so two copies would have drifted. BitBit Delay now uses it too.
- **`ee/fx/DelayTimeMap.h`** (was `bitbit-delay/src/TimeMap.h`) - both plugins run
  the same delay chain, so the same knob position has to mean the same time.
- **The tremolo Rate sweep** - three numbers, now in `TremoloConfig.h`, so BitBit
...
leaf names are BitBit Delay's own *exactly*, which is the mechanism that lets one
...
`BitBitAlpineWebEditor` contains no list of parameters at all. Forty-six relays
...
Any future WebView pedal gets its relays from this too, and BitBit Delay is now
...
**The meter event keeps BitBit Delay's name** (`"delayMeter"`). The face inside
the Delay module *is* BitBit Delay's face and listens for exactly that - one feed
...
**Refactoring three shipping pedals.** Stages 4, 5 and 6 change BitBit Trem-Pan,
BitBit Delay and BitBit Spring. Each is a *move*, and each has an existing or new
...
the `ee::ui` faces, not the WebView ones, so BitBit Alpine adds nothing to it —
...
**CPU.** BitBit Alpine runs a tape machine or a tremolo, a full delay chain with
...
- **Module Level vs module Mix.** The side modules carry both (a 24px Level in
  ...
  way.
- **Preset bank.** BitBit Alpine's factory presets are new content, not migrated.
  Six headline patches plus categorised ones, matching BitBit Delay's shape.
