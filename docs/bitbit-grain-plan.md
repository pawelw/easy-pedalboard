# BitBit Grain — implementation plan
...
Status: **built.** `plugins/bitbit-grain` compiles, the face renders, and
`scripts/dev-check.sh bitbit-grain` is green. What follows is the design as
...
[bitbit-reverb](../plugins/bitbit-reverb/src/PluginProcessor.cpp) around line 181.

Trails on bypass: copy bitbit-reverb's approach — stop feeding the network but
...
| `plugins/bitbit-grain/CMakeLists.txt` | `bitbit_add_plugin(BitBitGrain CODE Pgrn PRODUCT "BitBit Grain" BUNDLE com.synthpeak.peakgrain CATEGORIES "Fx")`. `Pgrn` does not collide with the nine existing codes. |
| `plugins/bitbit-grain/src/PluginProcessor.{h,cpp}` | Modelled on bitbit-chorus (engine does the work) plus bitbit-reverb's mono-send and trails handling. |
| `tests/GrainStress.cpp` | → `ee_grain_stress`. |

### Edited

- `CMakeLists.txt:102` — add `bitbit-grain` to `EE_ALL_PLUGINS`.
...
- Theme: `PedalTheme::pink()`, which was unused. (`gold()` was the first pick
  and turned out to be a sage green too close to BitBit Tape's.)
- No side panel. `ShimmerTunerPanel` is bitbit-reverb's and stays there.
...
**8.0 s**, and that constant is shared with bitbit-reverb — raising it would
silently change bitbit-reverb's knob range too.
...
4. `cmake --preset fast -DEE_PLUGINS="bitbit-grain"` and listen. Standalone is a
...
6. Mirror into `tests/UiSnapshot.cpp`, add `ee_grain_stress`, run
   `scripts/dev-check.sh bitbit-grain`, update `README.md` and `CLAUDE.md`.
...
already owned `kDefaultMixPct`. BitBit Grain's is `kDefaultGrainMixPct`. Worth
