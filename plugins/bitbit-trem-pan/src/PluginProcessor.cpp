...
/** A plain "ms" wordmark for the tempo-sync button, the same glyph BitBit Delay
...
BitBitTremPanProcessor::BitBitTremPanProcessor()
...
void BitBitTremPanProcessor::onSyncToggled()
{
...
juce::AudioProcessorValueTreeState::ParameterLayout BitBitTremPanProcessor::createParameterLayout()
{
...
void BitBitTremPanProcessor::prepareToPlay (double newSampleRate, int maximumExpectedSamplesPerBlock)
{
...
void BitBitTremPanProcessor::releaseResources() {}

bool BitBitTremPanProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
...
juce::String BitBitTremPanProcessor::rateReadout() const
{
    return ee::trempan::rateToText (rateParam->load(), syncParam->load() > 0.5f);
}

void BitBitTremPanProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
...
juce::AudioProcessorEditor* BitBitTremPanProcessor::createEditor()
{
    ...
    // Tempo-sync toggle centred above the Rate knob. It is BitBit Delay's own
...
void BitBitTremPanProcessor::setStateInformation (const void* data, int sizeInBytes)
{
...
{
    return new BitBitTremPanProcessor();
}
