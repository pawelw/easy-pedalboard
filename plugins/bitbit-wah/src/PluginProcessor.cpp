...
BitBitWahProcessor::BitBitWahProcessor()
      ...
      apvts (*this, nullptr, "PARAMETERS", createParameterLayout())
{
    ...
    storedFreeRate01.store (ee::bitbitwah::rate01ForFreePeriodMs (kDefaultFreePeriodMs));
...
void BitBitWahProcessor::parameterChanged (const juce::String& parameterID, float)
{
...
void BitBitWahProcessor::onSyncToggled()
{
...
juce::AudioProcessorValueTreeState::ParameterLayout BitBitWahProcessor::createParameterLayout()
{
    ...
    // (BitBit Wah runs this knob the other way up - see RateMap.h). The host-facing
    ...
    layout.add (std::make_unique<juce::AudioParameterFloat> (
        ...
        juce::AudioParameterFloatAttributes().withStringFromValueFunction (
            [] (float v, int) { return ee::bitbitwah::rateToText (v, true); })));
...
void BitBitWahProcessor::prepareToPlay (double newSampleRate, int maximumExpectedSamplesPerBlock)
{
...
void BitBitWahProcessor::releaseResources() {}

bool BitBitWahProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
...
juce::String BitBitWahProcessor::freqReadout() const
{
    return hzToText (freqHzFor (freqParam->load()), 0);
}

juce::String BitBitWahProcessor::timeReadout() const
{
    return ee::bitbitwah::rateToText (timeParam->load(), syncParam->load() > 0.5f);
}

juce::String BitBitWahProcessor::typeReadout() const
{
    return filterTypeToText (typeParam->load(), 0);
}

void BitBitWahProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
    ...
    const float periodSeconds = juce::jmax (1.0e-4f, ee::bitbitwah::rateToPeriodSeconds (time01, synced, bpm));
    ...
    if (synced && havePpq && isPlaying)
    {
        const double cyclesPerQuarter =
            1.0 / juce::jmax (1.0e-4, static_cast<double> (ee::bitbitwah::syncedDivisionBeats (time01)));
...
juce::AudioProcessorEditor* BitBitWahProcessor::createEditor()
{
    return new BitBitWahWebEditor (*this);
}
...
void BitBitWahProcessor::setStateInformation (const void* data, int sizeInBytes)
{
