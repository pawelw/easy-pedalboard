...
```bash
npm run build --prefix plugins/bitbit-wah/jsui
```
...
```bash
cmake --preset fast -DEE_PLUGINS="bitbit-wah" -DEE_JSUI_DEV_SERVER=ON
npm run dev --prefix plugins/bitbit-wah/jsui   # http://localhost:3000
```
