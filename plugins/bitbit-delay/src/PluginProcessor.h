...
class BitBitDelayProcessor : public juce::AudioProcessor, private juce::AudioProcessorValueTreeState::Listener
{
public:
    BitBitDelayProcessor();
    ~BitBitDelayProcessor() override;
    ...
    /** The pedal's preset banks: the one compiled in from plugins/bitbit-delay/
...
private:
    ...
    /** The pedal's whole chain: the delay line, a tape machine either side of
        ...
        the trims. BitBit Alpine's Delay module is a second instance of this, which
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (BitBitDelayProcessor)
};
