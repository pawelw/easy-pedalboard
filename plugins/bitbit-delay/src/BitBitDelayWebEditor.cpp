...
#if JUCE_ANDROID
const juce::String BitBitDelayWebEditor::devServerAddress = "http://10.0.2.2:3001/";
#else
const juce::String BitBitDelayWebEditor::devServerAddress = "http://localhost:3001/";
#endif

bool BitBitDelayWebEditor::SinglePageBrowser::pageAboutToLoad (const juce::String& newURL)
{
    return newURL == BitBitDelayWebEditor::devServerAddress || newURL == getResourceProviderRoot();
}

bool BitBitDelayWebEditor::SinglePageBrowser::pageLoadHadNetworkError (const juce::String&)
{
...
BitBitDelayWebEditor::BitBitDelayWebEditor (BitBitDelayProcessor& p)
      ...
      onAttachment (*p.apvts.getParameter ("on"), onRelay, p.apvts.undoManager)
{
    ...
    // fixed 526 plus .page's 4px each side), the height only a guess, so the
...
BitBitDelayWebEditor::~BitBitDelayWebEditor()
{
    stopTimer();
}

void BitBitDelayWebEditor::timerCallback()
{
...
{
    return ee::plugin::webface::serveFromDist (juce::File (BITBITDELAY_JSUI_DIR), url);
}
