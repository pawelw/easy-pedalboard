...
// same two strings BitBit EQ's corner knobs print, because these are the same
// pair of filters and should say the same thing about themselves.
juce::String loCutToText (float hz, int)
{
    return hz <= BitBitDelayProcessor::kLoCutMinHz + 0.5f ? "0 Hz" : freqToText (hz);
}

juce::String hiCutToText (float hz, int)
{
    return hz >= BitBitDelayProcessor::kHiCutMaxHz - 0.5f ? infinitySymbol() : freqToText (hz);
}
} // namespace

BitBitDelayProcessor::BitBitDelayProcessor()
...
bool BitBitDelayProcessor::isSynced() const
{
    return timeUnitParam == nullptr || timeUnitParam->load() < 0.5f;
}

ee::dsp::TapeDelay::Routing BitBitDelayProcessor::routing() const noexcept
{
...
juce::AudioProcessorValueTreeState::ParameterLayout BitBitDelayProcessor::createParameterLayout()
{
    ...
    // BitBit Trem & Pan's Rate does; the editor overrides it live off the pill.
    ...
    // The Filter section. Ranges, skews and defaults are BitBit EQ's, down to the
...
double BitBitDelayProcessor::readPlayHeadBpm()
{
    double bpm = 120.0;
...
double BitBitDelayProcessor::currentBpm() const
{
    return lastKnownBpm.load (std::memory_order_relaxed);
}

juce::String BitBitDelayProcessor::timeReadout (const std::atomic<float>* timeParam) const
{
    const float time01 = timeParam != nullptr ? timeParam->load() : kDefaultTime01;
    return ee::bitbitdelay::timeMap().toText (time01, isSynced(), currentBpm());
}

float BitBitDelayProcessor::timeMs (const std::atomic<float>* timeParam) const
{
    const float time01 = timeParam != nullptr ? timeParam->load() : kDefaultTime01;
    return ee::bitbitdelay::timeMap().value (time01, isSynced(), currentBpm());
}

juce::String BitBitDelayProcessor::timeMsReadout (const std::atomic<float>* timeParam) const
{
    return juce::String (juce::roundToInt (timeMs (timeParam))) + " ms";
}

void BitBitDelayProcessor::mirrorTime (const juce::String& from, const juce::String& to)
{
...
/** Installs a whole APVTS tree - a host restoring a session, a preset load -
    ...
    time before its "off" ever arrived. BitBit Delay ships three presets like
...
void BitBitDelayProcessor::installState (const juce::ValueTree& tree)
{
...
void BitBitDelayProcessor::parameterChanged (const juce::String& parameterID, float newValue)
{
...
void BitBitDelayProcessor::prepareToPlay (double sampleRate, int maximumExpectedSamplesPerBlock)
{
...
void BitBitDelayProcessor::releaseResources()
{
    chain.reset();
}

double BitBitDelayProcessor::getTailLengthSeconds() const
{
    return chain.tailSeconds();
}

bool BitBitDelayProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
...
void BitBitDelayProcessor::pushSettings (double bpm, bool synced) noexcept
{
    chain.setTimes (ee::bitbitdelay::timeSeconds (leftTimeParam->load(), synced, bpm),
...
void BitBitDelayProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
...
{
    return new BitBitDelayWebEditor (*this);
}
...
void BitBitDelayProcessor::setStateInformation (const void* data, int sizeInBytes)
{
...
{
    return new BitBitDelayProcessor();
}
