...
BitBitPhaseProcessor::BitBitPhaseProcessor()
...
juce::AudioProcessorValueTreeState::ParameterLayout BitBitPhaseProcessor::createParameterLayout()
{
    ...
    auto rateRange = juce::NormalisableRange<float> (ee::dsp::phaser::kRateMinHz, ee::dsp::phaser::kRateMaxHz);
    rateRange.setSkewForCentre (ee::dsp::phaser::kRateSkewCentreHz);
    layout.add (std::make_unique<juce::AudioParameterFloat> (
        juce::ParameterID { kRateID, 1 }, "Rate", rateRange, ee::dsp::phaser::kDefaultRateHz,
...
void BitBitPhaseProcessor::prepareToPlay (double newSampleRate, int maximumExpectedSamplesPerBlock)
{
...
void BitBitPhaseProcessor::releaseResources() {}

bool BitBitPhaseProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
...
juce::String BitBitPhaseProcessor::rateReadout() const
{
    return hzToText (rateParam->load(), 0);
}

void BitBitPhaseProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
...
juce::AudioProcessorEditor* BitBitPhaseProcessor::createEditor()
{
...
void BitBitPhaseProcessor::setStateInformation (const void* data, int sizeInBytes)
{
...
{
    return new BitBitPhaseProcessor();
}
