...
juce::String loCutToText (float hz, int)
{
    ...
    return hz <= BitBitEqProcessor::kLoCutMinHz + 0.5f ? "0 Hz" : freqToText (hz);
}

juce::String hiCutToText (float hz, int)
{
    return hz >= BitBitEqProcessor::kHiCutMaxHz - 0.5f ? infinitySymbol() : freqToText (hz);
}
...
BitBitEqProcessor::BitBitEqProcessor()
...
juce::AudioProcessorValueTreeState::ParameterLayout BitBitEqProcessor::createParameterLayout()
{
...
void BitBitEqProcessor::updateFilters (bool force)
{
...
void BitBitEqProcessor::prepareToPlay (double newSampleRate, int maximumExpectedSamplesPerBlock)
{
...
void BitBitEqProcessor::releaseResources()
{
    for (auto& chain : filters)
        for (auto& filter : chain)
            filter.reset();
...
bool BitBitEqProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
...
void BitBitEqProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
...
juce::AudioProcessorEditor* BitBitEqProcessor::createEditor()
{
    ...
    // Shares BitBit Delay's theme so the two pedals sit together on a rack.
...
void BitBitEqProcessor::setStateInformation (const void* data, int sizeInBytes)
{
