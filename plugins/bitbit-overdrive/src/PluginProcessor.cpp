...
BitBitOverdriveProcessor::BitBitOverdriveProcessor()
...
juce::AudioProcessorValueTreeState::ParameterLayout BitBitOverdriveProcessor::createParameterLayout()
{
    ...
    layout.add (std::make_unique<juce::AudioParameterFloat> (juce::ParameterID { kToneID, 1 }, "Tone", percent,
                                                             ee::dsp::config::kDefaultTone01 * 100.0f,
                                                             percentAttributes));

    layout.add (std::make_unique<juce::AudioParameterFloat> (juce::ParameterID { kDriveID, 1 }, "Drive", percent,
                                                             ee::dsp::config::kDefaultDrive01 * 100.0f,
                                                             percentAttributes));
...
void BitBitOverdriveProcessor::prepareToPlay (double newSampleRate, int maximumExpectedSamplesPerBlock)
{
    ...
    levelGain.setCurrentAndTargetValue (juce::Decibels::decibelsToGain (levelParam->load()));
...
void BitBitOverdriveProcessor::releaseResources() {}

bool BitBitOverdriveProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
...
void BitBitOverdriveProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
...
juce::AudioProcessorEditor* BitBitOverdriveProcessor::createEditor()
{
    ...
    // footprint of BitBit Reverb (two knob columns), not a wide three-across face.
    spec.knobs = {
        { kLevelID, "Level" },
        { kDriveID, "Drive" },
        { kToneID, "Tone" },
...
void BitBitOverdriveProcessor::setStateInformation (const void* data, int sizeInBytes)
{
...
{
    return new BitBitOverdriveProcessor();
}
