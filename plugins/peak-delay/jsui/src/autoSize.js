...
// The plugin's window has been sized by hand from a Chromium measurement of
// the panel before (see Peak Wah's own note on this) and a real WKWebView
// rendered it taller than that number - font/line-height metrics differ
// enough between the two engines that there's no way to get this right by
// guessing from outside a real host. This has the page measure its own
// actual rendered size and tell PeakDelayWebEditor to match it exactly, so
// it's never a guess regardless of engine, font-loading timing, or future
// content changes.
