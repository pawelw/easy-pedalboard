...
juce::String loCutToText (float hz, int)
{
    ...
    return hz <= PeakEqProcessor::kLoCutMinHz + 0.5f ? "0 Hz" : freqToText (hz);
}

juce::String hiCutToText (float hz, int)
{
    return hz >= PeakEqProcessor::kHiCutMaxHz - 0.5f ? infinitySymbol() : freqToText (hz);
}

// Band parameter IDs, low to high, in the same order as
// PeakEqProcessor::kBandFrequencies.
constexpr std::array<const char*, PeakEqProcessor::kNumBands> kBandIDs { { "b100", "b200", "b400", "b800", "b1k6",
                                                                           "b3k2", "b6k4" } };

// Captions printed under each fader.
constexpr std::array<const char*, PeakEqProcessor::kNumBands> kBandCaptions { { "100", "200", "400", "800", "1.6k",
                                                                                "3.2k", "6.4k" } };
...
PeakEqProcessor::PeakEqProcessor()
      ...
      apvts (*this, nullptr, "PARAMETERS", createParameterLayout())
{
    for (int i = 0; i < kNumBands; ++i)
        bandParams[static_cast<size_t> (i)] = apvts.getRawParameterValue (kBandIDs[static_cast<size_t> (i)]);
...
juce::AudioProcessorValueTreeState::ParameterLayout PeakEqProcessor::createParameterLayout()
{
    ...
    layout.add (std::make_unique<juce::AudioParameterFloat> (juce::ParameterID { kLevelID, 1 }, "Level", dbRange, 0.0f,
                                                             dbAttributes));

    const char* bandNames[kNumBands] = { "100 Hz", "200 Hz", "400 Hz", "800 Hz", "1.6 kHz", "3.2 kHz", "6.4 kHz" };

    for (int i = 0; i < kNumBands; ++i)
        layout.add (std::make_unique<juce::AudioParameterFloat> (
            juce::ParameterID { kBandIDs[static_cast<size_t> (i)], 1 }, bandNames[i], dbRange, 0.0f, dbAttributes));
...
void PeakEqProcessor::updateFilters (bool force)
{
    for (int b = 0; b < kNumBands; ++b)
    {
        ...
        auto coeffs = juce::dsp::IIR::Coefficients<float>::makePeakFilter (
            sampleRate, kBandFrequencies[static_cast<size_t> (b)], kBandQ, juce::Decibels::decibelsToGain (db));
...
void PeakEqProcessor::prepareToPlay (double newSampleRate, int maximumExpectedSamplesPerBlock)
{
    ...
    levelGain.setCurrentAndTargetValue (engaged ? juce::Decibels::decibelsToGain (levelParam->load()) : 1.0f);

    wetMix.reset (newSampleRate, kRampSeconds);
    wetMix.setCurrentAndTargetValue (engaged ? 1.0f : 0.0f);
}

void PeakEqProcessor::releaseResources()
{
    for (auto& chain : filters)
        for (auto& filter : chain)
            filter.reset();
...
bool PeakEqProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
...
void PeakEqProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
    ...
    updateFilters (false);
    levelGain.setTargetValue (engaged ? juce::Decibels::decibelsToGain (levelParam->load()) : 1.0f);
...
juce::AudioProcessorEditor* PeakEqProcessor::createEditor()
{
    ...
    spec.sliders.push_back (
        { .parameterID = kLevelID, .caption = "LEVEL", .fill = juce::Colour (0xffd6d6d6), .joinCurve = false });
    for (int i = 0; i < kNumBands; ++i)
    ...
    spec.cornerKnobs = {
        ...
        { .parameterID = kHiCutID, .compact = true, .cutSide = ee::ui::CutSide::high, .invertedArc = true },
    };
    ...
    // Shares Peak Delay's theme so the two pedals sit together on a rack.
    return new ee::ui::PedalEditor (*this, apvts, spec, ee::ui::PedalTheme::silver());
}

void PeakEqProcessor::getStateInformation (juce::MemoryBlock& destData)
{
    if (auto xml = apvts.copyState().createXml())
        copyXmlToBinary (*xml, destData);
}

void PeakEqProcessor::setStateInformation (const void* data, int sizeInBytes)
{
