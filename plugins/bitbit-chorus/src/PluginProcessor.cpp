...
BitBitChorusProcessor::BitBitChorusProcessor()
...
juce::AudioProcessorValueTreeState::ParameterLayout BitBitChorusProcessor::createParameterLayout()
{
    ...
    auto rateRange = juce::NormalisableRange<float> (ee::dsp::config::kRateMinHz, ee::dsp::config::kRateMaxHz);
    rateRange.setSkewForCentre (ee::dsp::config::kRateSkewCentreHz);
    layout.add (std::make_unique<juce::AudioParameterFloat> (
        juce::ParameterID { kRateID, 1 }, "Rate", rateRange, ee::dsp::config::kDefaultRateHz,
        juce::AudioParameterFloatAttributes().withStringFromValueFunction (hzToText)));

    layout.add (std::make_unique<juce::AudioParameterFloat> (juce::ParameterID { kDepthID, 1 }, "Depth", percent,
...
void BitBitChorusProcessor::prepareToPlay (double newSampleRate, int maximumExpectedSamplesPerBlock)
{
...
void BitBitChorusProcessor::releaseResources() {}

bool BitBitChorusProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
...
juce::String BitBitChorusProcessor::rateReadout() const
{
    return hzToText (rateParam->load(), 0);
}

void BitBitChorusProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
...
juce::AudioProcessorEditor* BitBitChorusProcessor::createEditor()
{
    ...
    // Two rows of two, same footprint as BitBit Reverb.
    spec.knobsPerRow = 2;
...
void BitBitChorusProcessor::setStateInformation (const void* data, int sizeInBytes)
{
...
{
    return new BitBitChorusProcessor();
}
