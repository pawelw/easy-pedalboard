...
BitBitTapeProcessor::BitBitTapeProcessor()
...
void BitBitTapeProcessor::loadNoiseSample()
{
...
juce::AudioProcessorValueTreeState::ParameterLayout BitBitTapeProcessor::createParameterLayout()
{
    ...
    // How tired the tape is. This is BitBit Delay's Tape stage - the same engine
...
void BitBitTapeProcessor::prepareToPlay (double sampleRate, int maximumExpectedSamplesPerBlock)
{
...
void BitBitTapeProcessor::releaseResources()
{
    machine.reset();
}

bool BitBitTapeProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
...
void BitBitTapeProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
...
juce::AudioProcessorEditor* BitBitTapeProcessor::createEditor()
{
    ...
    // edges - so the face is a whole column narrower than BitBit Delay's.
    spec.knobs = {
        { kSaturationID, "Saturation" }, { kFlutterID, "Flutter" }, { kWearID, "Wear" }, { kNoiseID, "Noise" }
    };

    // Tone gets BitBit Reverb's RESO treatment: a small vector cap dead centre of
    ...
    // Width is one thing or the other, so it is the big sliding switch BitBit
    ...
    // than BitBit Reverb's RESO does, which leaves the height for the caps.
    spec.knobRowGap = 12;

    spec.knobsPerRow = 2;

    // Same width and (default) cap size as BitBit Reverb, so the two faces line up
    // on a rack.
    ...
    // bezel ring, the same fix BitBit Reverb uses on its own dark-ish face.
    ...
    // silver bezel, which is why these caps read differently from BitBit Reverb's.
...
void BitBitTapeProcessor::setStateInformation (const void* data, int sizeInBytes)
{
...
{
    return new BitBitTapeProcessor();
}
