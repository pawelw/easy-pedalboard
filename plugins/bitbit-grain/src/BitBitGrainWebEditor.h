...
class BitBitGrainProcessor;

/** BitBit Grain's face: the same juce::WebBrowserComponent + React arrangement
    BitBit Reverb uses (ee::plugin::RelaySet, which walks the processor's own
    ...
    Two things ride along on top of RelaySet, both lifted from BitBit Delay's
    ...
    face touched. */
class BitBitGrainWebEditor : public juce::AudioProcessorEditor, private juce::Timer
{
public:
    explicit BitBitGrainWebEditor (BitBitGrainProcessor&);
    ~BitBitGrainWebEditor() override;
...
private:
    ...
    BitBitGrainProcessor& processorRef;
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (BitBitGrainWebEditor)
};
