...
class BitBitReverbProcessor;

/** BitBit Reverb's face: the same juce::WebBrowserComponent + React arrangement
    BitBit Artifact uses.
...
class BitBitReverbWebEditor : public juce::AudioProcessorEditor
{
public:
    explicit BitBitReverbWebEditor (BitBitReverbProcessor&);
    ~BitBitReverbWebEditor() override;
...
private:
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (BitBitReverbWebEditor)
};
