...
/** A bare rounded "440 Hz", no kHz and no decimal - the Filter Freq readout,
    the same text Peak Alpine prints for the same knob. Not the shared hzToText,
...
PeakModulationProcessor::PeakModulationProcessor()
...
PeakModulationProcessor::~PeakModulationProcessor() = default;

/** Decodes the embedded tape floor once, at construction, exactly as
    PeakTapeProcessor and PeakAlpineProcessor do. */
void PeakModulationProcessor::loadTapeNoiseSample()
{
...
juce::AudioProcessorValueTreeState::ParameterLayout PeakModulationProcessor::createParameterLayout()
{
    juce::AudioProcessorValueTreeState::ParameterLayout layout;

    // Every id, range and default is Peak Alpine's Modulation module's, minus
    ...
    // Tape. Ranges and defaults are Peak Tape's.
    ...
    // Tremolo. Rate is a plain 0..1 knob, like Peak Trem & Pan's: what a
    ...
    // Chorus and Phaser. Peak Chorus's and Peak Phase's ranges and defaults.
...
juce::String PeakModulationProcessor::filterTimeReadout() const
{
...
juce::String PeakModulationProcessor::tremoloRateReadout() const
{
...
void PeakModulationProcessor::pushSettings (double blockBpm) noexcept
{
...
void PeakModulationProcessor::installState (const juce::ValueTree& tree)
{
    apvts.replaceState (tree);
}

double PeakModulationProcessor::readPlayHeadBpm()
{
...
void PeakModulationProcessor::prepareToPlay (double newSampleRate, int maximumExpectedSamplesPerBlock)
{
...
void PeakModulationProcessor::releaseResources()
{
    module.reset();
}

bool PeakModulationProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
...
void PeakModulationProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
...
juce::AudioProcessorEditor* PeakModulationProcessor::createEditor()
{
    return new PeakModulationWebEditor (*this);
}

void PeakModulationProcessor::getStateInformation (juce::MemoryBlock& destData)
{
    if (auto xml = apvts.copyState().createXml())
        copyXmlToBinary (*xml, destData);
}

void PeakModulationProcessor::setStateInformation (const void* data, int sizeInBytes)
{
...
{
    return new PeakModulationProcessor();
}
