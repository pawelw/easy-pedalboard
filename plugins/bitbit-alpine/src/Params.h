...
/** BitBit Alpine's parameter ids, namespaced by module.
 *
 ...
 * drive BitBit Delay's bare `mix` and this plugin's `dly.mix` without knowing
 * which host it is in.
 *
 ...
 * `rev.level` are, and they are driven from BitBit Alpine's own header rather than
...
// ----------------------------------------------------------------- artifact
// The first module: ee::fx::ArtifactModule, which is what BitBit Artifact runs.
...
// BitBit Wah's engine as a swept filter - it was BitBit Artifact's `flt.` engine
// until it moved here.
...
inline constexpr const char* dlyLevel = "dly.level"; // module chrome, not BitBit Delay's
