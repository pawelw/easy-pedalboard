...
EasyReverbProcessor::createParameterLayout() {
  juce::AudioProcessorValueTreeState::ParameterLayout layout;

  auto decayRange = juce::NormalisableRange<float>(
      ee::dsp::FdnReverb::kMinDecay, ee::dsp::FdnReverb::kMaxDecay);
  ...
  auto lowCutRange = juce::NormalisableRange<float>(
      ee::dsp::FdnReverb::kMinLowCutHz, ee::dsp::FdnReverb::kMaxLowCutHz);
...
void EasyReverbProcessor::prepareToPlay(double sampleRate,
  ...
  wetGain.setCurrentAndTargetValue(
      std::sin(mix * juce::MathConstants<float>::halfPi) * kWetTrim);
  inputGain.setCurrentAndTargetValue(engaged ? 1.0f : 0.0f);
}

void EasyReverbProcessor::releaseResources() { reverb.reset(); }

double EasyReverbProcessor::getTailLengthSeconds() const {
  return static_cast<double>(reverb.getTailSeconds());
}

bool EasyReverbProcessor::isBusesLayoutSupported(
...
void EasyReverbProcessor::processBlock(juce::AudioBuffer<float> &buffer,
                                       juce::MidiBuffer &) {
  ...
  const int numOut =
      juce::jmin(getTotalNumOutputChannels(), buffer.getNumChannels());
...
void EasyReverbProcessor::setStateInformation(const void *data,
                                              int sizeInBytes) {
