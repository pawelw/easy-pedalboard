...
PeakReverbProcessor::createParameterLayout() {
...
void PeakReverbProcessor::prepareToPlay(double sampleRate,
...
void PeakReverbProcessor::releaseResources() { reverb.reset(); }

double PeakReverbProcessor::getTailLengthSeconds() const {
  return static_cast<double>(reverb.getTailSeconds());
}

bool PeakReverbProcessor::isBusesLayoutSupported(
...
void PeakReverbProcessor::processBlock(juce::AudioBuffer<float> &buffer,
                                       juce::MidiBuffer &) {
...
void PeakReverbProcessor::setStateInformation(const void *data,
                                              int sizeInBytes) {
