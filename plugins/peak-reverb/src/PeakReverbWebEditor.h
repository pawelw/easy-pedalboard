...
class PeakReverbProcessor;

/** Peak Reverb's face: the same juce::WebBrowserComponent + React arrangement
    Peak Artifact uses.
...
class PeakReverbWebEditor : public juce::AudioProcessorEditor
{
public:
    explicit PeakReverbWebEditor (PeakReverbProcessor&);
    ~PeakReverbWebEditor() override;
...
private:
    ...
    PeakReverbProcessor& processorRef;
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (PeakReverbWebEditor)
};
