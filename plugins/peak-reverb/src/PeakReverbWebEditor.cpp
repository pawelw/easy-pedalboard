...
#if JUCE_ANDROID
const juce::String PeakReverbWebEditor::devServerAddress = "http://10.0.2.2:3005/";
#else
const juce::String PeakReverbWebEditor::devServerAddress = "http://localhost:3005/";
#endif

bool PeakReverbWebEditor::SinglePageBrowser::pageAboutToLoad (const juce::String& newURL)
{
    return newURL == PeakReverbWebEditor::devServerAddress || newURL == getResourceProviderRoot();
}

bool PeakReverbWebEditor::SinglePageBrowser::pageLoadHadNetworkError (const juce::String&)
{
    if (triedFallback)
        return true;
...
PeakReverbWebEditor::PeakReverbWebEditor (PeakReverbProcessor& p)
...
PeakReverbWebEditor::~PeakReverbWebEditor() = default;

void PeakReverbWebEditor::resized()
{
    webView.setBounds (getLocalBounds());
}

std::optional<juce::WebBrowserComponent::Resource> PeakReverbWebEditor::getResource (const juce::String& url)
{
    return ee::plugin::webface::serveFromDist (juce::File (PEAKREVERB_JSUI_DIR), url);
}
