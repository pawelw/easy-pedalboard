...
EasyOverdriveProcessor::EasyOverdriveProcessor()
    : juce::AudioProcessor(
          BusesProperties()
...
EasyOverdriveProcessor::createParameterLayout() {
...
void EasyOverdriveProcessor::prepareToPlay(double newSampleRate,
...
void EasyOverdriveProcessor::releaseResources() {}

bool EasyOverdriveProcessor::isBusesLayoutSupported(
...
void EasyOverdriveProcessor::processBlock(juce::AudioBuffer<float> &buffer,
                                          juce::MidiBuffer &) {
...
juce::AudioProcessorEditor *EasyOverdriveProcessor::createEditor() {
  ...
  // footprint of Easy Reverb (two knob columns), not a wide three-across face.
  spec.knobs = {
      {kLevelID, "Level"},
      {kDriveID, "Drive"},
      {kToneID, "Tone"},
...
void EasyOverdriveProcessor::getStateInformation(juce::MemoryBlock &destData) {
  if (auto xml = apvts.copyState().createXml())
    copyXmlToBinary(*xml, destData);
}

void EasyOverdriveProcessor::setStateInformation(const void *data,
                                                 int sizeInBytes) {
...
juce::AudioProcessor *JUCE_CALLTYPE createPluginFilter() {
  return new EasyOverdriveProcessor();
}
