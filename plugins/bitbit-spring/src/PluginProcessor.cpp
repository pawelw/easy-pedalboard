...
// Mix knob shaping, the same curve BitBit Reverb uses: above 1.0 the wet comes in
...
BitBitSpringProcessor::BitBitSpringProcessor()
...
juce::AudioProcessorValueTreeState::ParameterLayout BitBitSpringProcessor::createParameterLayout()
{
...
void BitBitSpringProcessor::prepareToPlay (double sampleRate, int maximumExpectedSamplesPerBlock)
{
...
void BitBitSpringProcessor::releaseResources()
{
    spring.reset();
}

double BitBitSpringProcessor::getTailLengthSeconds() const
{
    return static_cast<double> (spring.getTailSeconds());
}

bool BitBitSpringProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
...
void BitBitSpringProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
...
juce::AudioProcessorEditor* BitBitSpringProcessor::createEditor()
{
    ...
    // One knob per row - the same narrow two-control face as BitBit Phase.
    spec.knobsPerRow = 1;
...
void BitBitSpringProcessor::setStateInformation (const void* data, int sizeInBytes)
{
...
{
    return new BitBitSpringProcessor();
}
