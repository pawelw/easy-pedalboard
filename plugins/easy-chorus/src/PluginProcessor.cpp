...
EasyChorusProcessor::EasyChorusProcessor()
    : juce::AudioProcessor(
          BusesProperties()
...
EasyChorusProcessor::createParameterLayout() {
...
void EasyChorusProcessor::prepareToPlay(double newSampleRate,
...
void EasyChorusProcessor::releaseResources() {}

bool EasyChorusProcessor::isBusesLayoutSupported(
...
juce::String EasyChorusProcessor::rateReadout() const {
  return hzToText(rateParam->load(), 0);
}

void EasyChorusProcessor::processBlock(juce::AudioBuffer<float> &buffer,
                                       juce::MidiBuffer &) {
...
juce::AudioProcessorEditor *EasyChorusProcessor::createEditor() {
  ...
  // Two rows of two, same footprint as Easy Reverb.
  spec.knobsPerRow = 2;
...
void EasyChorusProcessor::getStateInformation(juce::MemoryBlock &destData) {
  if (auto xml = apvts.copyState().createXml())
    copyXmlToBinary(*xml, destData);
}

void EasyChorusProcessor::setStateInformation(const void *data, int sizeInBytes) {
...
juce::AudioProcessor *JUCE_CALLTYPE createPluginFilter() {
  return new EasyChorusProcessor();
}
