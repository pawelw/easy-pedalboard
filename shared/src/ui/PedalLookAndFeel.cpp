...
namespace
{
    ...
    juce::Image spoonImage()
    {
        static const juce::Image img =
            juce::ImageCache::getFromMemory (BinaryData::spoon_png, BinaryData::spoon_pngSize);
        return img;
    }
    ...
    {
        static const juce::Image img = juce::ImageCache::getFromMemory (BinaryData::knob_png, BinaryData::knob_pngSize);
        return img;
    }
    ...
    {
        static const juce::Image img =
            juce::ImageCache::getFromMemory (BinaryData::plate_png, BinaryData::plate_pngSize);
        return img;
    }
    ...
    //== analogSilver: a black cap recessed in a static brushed-silver bezel. ====

    /** The bezel ring the silver cap sits in: a brushed-silver washer with a
        transparent centre, never rotated. Its inner hole is `0.77` of the
        artwork radius. Loaded once. */
    juce::Image silverBezelImage()
    {
        static const juce::Image img =
            juce::ImageCache::getFromMemory (BinaryData::silverknobbasev1_png, BinaryData::silverknobbasev1_pngSize);
        return img;
    }

    /** The silver style's spinning cap: `knob.png` with its outer black collar
        removed, so just the fluted body, brushed metal centre and a pointer at
        12 o'clock, filling the frame. Turned by the absolute `angle`, like the
        analog cap. Loaded once. */
    juce::Image silverCapImage()
    {
        static const juce::Image img =
            juce::ImageCache::getFromMemory (BinaryData::silverknob_png, BinaryData::silverknob_pngSize);
        return img;
    }
    ...
    /** The static centre plate plus its orbiting glint and a screen-space
        ...
        and silver caps, which differ only in the cap under it and the plate
        size. `glintAngle` orbits the specular highlight. */
    void drawKnobPlateAndSheen (
        juce::Graphics& g, juce::Point<float> centre, float R, float glintAngle, float plateRadiusFrac)
    {
        ...
        {
            const float plateR = R * plateRadiusFrac;
            const float ps = (plateR * 2.0f) / static_cast<float> (plate.getWidth());
            const auto pt = juce::AffineTransform::translation (plate.getWidth() * -0.5f, plate.getHeight() * -0.5f)
                                .scaled (ps)
                                .rotated (glintAngle * kPlateSpinFactor)
            ...
            const float ga = glintAngle * kPlateGlintSpinFactor;
            ...
            juce::ColourGradient gg (juce::Colours::white.withAlpha (kPlateGlintAlpha), gp.x, gp.y,
                                     juce::Colours::transparentWhite, gp.x, gp.y + plateR * 0.9f, true);
        ...
        {
            const auto lit = juce::Rectangle<float> (R * 2.0f, R * 2.0f).withCentre (centre).reduced (R * 0.06f);
            juce::Graphics::ScopedSaveState state (g);
            juce::Path clip;
            clip.addEllipse (lit);
            g.reduceClipRegion (clip);

            juce::ColourGradient sheen (juce::Colours::white.withAlpha (0.16f), lit.getCentreX(), lit.getY(),
                                        juce::Colours::transparentWhite, lit.getCentreX(),
                                        lit.getCentreY() + lit.getHeight() * 0.18f, false);
            g.setGradientFill (sheen);
            g.fillEllipse (lit);

            juce::ColourGradient shade (
                juce::Colours::transparentBlack, lit.getCentreX(), lit.getCentreY() + lit.getHeight() * 0.2f,
                juce::Colours::black.withAlpha (0.13f), lit.getCentreX(), lit.getBottom(), false);
    ...
    void drawImageKnob (juce::Graphics& g,
                        ...
                        juce::Colour backing)
    {
        ...
        const auto t = juce::AffineTransform::translation (img.getWidth() * -0.5f, img.getHeight() * -0.5f)
                           .scaled (scale)
                           .rotated (angle)
                           .translated (centre.x, centre.y);
        ...
        drawKnobPlateAndSheen (g, centre, R, angle, kPlateRadiusFrac);
    }

    /** The `analogSilver` cap: a static brushed-silver bezel ring in place of
        the analog cap's black collar, the fluted cap turned to `angle` inside
        it, then the shared centre plate and lights on top. `R` is the nominal
        knob radius (as passed to `drawImageKnob`); the ring's outer edge lands
        at the same place `knob.png`'s does, so the control is a normal knob
        size. */
    void drawSilverKnob (juce::Graphics& g, juce::Point<float> centre, float R, float angle, const PedalTheme& theme)
    {
        const auto bezel = silverBezelImage();
        const auto cap = silverCapImage();

        // Opaque disc under the whole stack, so the sampler's transparent
        // surround cannot let the face colour through at either rim.
...

        {
            ...
            // Bezel: never rotates - it is the collar the cap is recessed into.
            if (bezel.isValid())
            {
                const float outerR = R * kSilverBezelOuterFrac;
                const float bs = (outerR * 2.0f) / static_cast<float> (bezel.getWidth());
                const auto bt = juce::AffineTransform::translation (bezel.getWidth() * -0.5f, bezel.getHeight() * -0.5f)
                                    .scaled (bs)
                                    .translated (centre.x, centre.y);
                g.drawImageTransformed (bezel, bt, false);
            }

            // Cap: fills its frame, so scale its edge to where the flutes should
            // end. Pointer is at 12 o'clock, so it turns by the absolute angle.
            if (cap.isValid())
            {
                const float cs = (R * kSilverCapReachFrac) / (static_cast<float> (cap.getWidth()) * 0.5f);
                const auto ct = juce::AffineTransform::translation (cap.getWidth() * -0.5f, cap.getHeight() * -0.5f)
                                    .scaled (cs)
                                    .rotated (angle)
                                    .translated (centre.x, centre.y);
                g.drawImageTransformed (cap, ct, false);
            }
        }

        drawKnobPlateAndSheen (g, centre, R, angle, kSilverPlateRadiusFrac);
    }
    ...
    void drawSpoonPointer (juce::Graphics& g, juce::Point<float> centre, float angle, float length)
    {
        ...
        const juce::Point<float> at (centre.x + distance * std::sin (angle), centre.y - distance * std::cos (angle));

        const auto place = juce::AffineTransform::translation (img.getWidth() * -0.5f, img.getHeight() * -0.5f)
                               .scaled (scale, scale)
                               .rotated (angle)
    ...
    void drawVectorKnob (
        juce::Graphics& g, juce::Point<float> centre, float R, float angle, const PedalTheme& theme, bool enabled)
    {
        using juce::Colour;
        ...
        // ---- outer black disc -------------------------------------
        {
            juce::ColourGradient gd (Colour (0xff272727), centre.x, centre.y - R, Colour (0xff0c0c0c), centre.x,
                                     centre.y + R, false);
        ...
        // gentle top-to-bottom shading on the cog face
        {
            ...
            juce::ColourGradient dk (Colours::transparentBlack, centre.x, centre.y, Colours::black.withAlpha (0.28f),
                                     centre.x, centre.y + cogTip, false);
        ...
        // ---- brushed-metal centre -----------------------------
        {
            ...
            juce::ColourGradient base (Colour (0xfff0f0f0), centre.x, centre.y, Colour (0xffa4a4a4), centre.x,
                                       centre.y + metalR, true);
            ...
            // spun sheen: two bright sweeps, two dark, via thin wedges
            {
                ...
                for (int i = 0; i < wedges; ++i)
                {
                    ...
                    float b = 0.64f + 0.26f * std::cos (2.0f * (a0 + 0.55f)) + 0.07f * std::cos (a0 - 0.3f);
                ...
                for (int i = 1; i <= 46; ++i)
                {
                    ...
                    g.setColour ((i % 2 ? Colours::white : Colours::black).withAlpha (0.03f + 0.02f * rng.nextFloat()));
        ...
        {
            juce::Path mk;
            mk.addRoundedRectangle (-markW * 0.5f, -(markOut - markIn), markW, markOut - markIn, markW * 0.28f);
            const auto t = juce::AffineTransform::rotation (angle).translated (pol ((markIn + markOut) * 0.5f, angle));
...
PedalLookAndFeel::PedalLookAndFeel (PedalTheme themeToUse) : theme (std::move (themeToUse))
{
    applyColours();
}
                                         ...
                                         juce::Slider& slider)
{
    ...
    const auto stroke = juce::PathStrokeType (track, juce::PathStrokeType::curved, juce::PathStrokeType::rounded);
    ...
    if (inverted)
    {
        ...
        const auto haloStroke =
            juce::PathStrokeType (track + 4.0f, juce::PathStrokeType::curved, juce::PathStrokeType::rounded);
        const auto whiteStroke =
            juce::PathStrokeType (track + 2.0f, juce::PathStrokeType::curved, juce::PathStrokeType::rounded);
    ...
    const auto arcColour = arcOverridden ? slider.findColour (juce::Slider::thumbColourId) : theme.accent;
    ...
    if (inverted)
    {
        ...
        {
            juce::Path value;
            value.addCentredArc (centre.x, centre.y, arcRadius, arcRadius, 0.0f, angle, rotaryEndAngle, true);
    ...
    else if (bipolar)
    {
        ...
        g.drawLine (juce::Line<float> (centre + tickDirection * tickInner, centre + tickDirection * tickOuter), 1.6f);

        if (std::abs (angle - centreAngle) > 0.001f)
        {
            juce::Path value;
            value.addCentredArc (centre.x, centre.y, arcRadius, arcRadius, 0.0f, juce::jmin (centreAngle, angle),
                                 juce::jmax (centreAngle, angle), true);
    ...
    // Full-size knobs get artwork or the vector cap; compact utility caps keep
    // the small ring-and-dot below. The value arc is already drawn underneath,
    // so a transparent cap keeps its ring.
    const bool compactKnob = static_cast<bool> (slider.getProperties().getWithDefault ("compactKnob", false));

    // The silver style swaps the cap alone: the fluted knob recessed in a
    // static brushed-silver bezel where the analog cap's black collar would be,
    // then the same centre plate and lights.
    if (theme.controlStyle == ControlStyle::analogSilver && ! compactKnob && silverBezelImage().isValid() &&
        silverCapImage().isValid())
    {
        drawSilverKnob (g, centre, arcRadius * kKnobSizeFrac, angle, theme);
        return;
    }
    ...
    if (theme.knobFilmstrip.isValid() && theme.knobFilmstripFrames > 1)
    {
        ...
        const int frame =
            juce::jlimit (0, theme.knobFilmstripFrames - 1,
                          juce::roundToInt (sliderPos * static_cast<float> (theme.knobFilmstripFrames - 1)));

        g.drawImage (theme.knobFilmstrip, body.getX(), body.getY(), body.getWidth(), body.getHeight(), 0,
                     frame * frameHeight, theme.knobFilmstrip.getWidth(), frameHeight);
        return;
    }
    ...
    if (theme.knobImage.isValid())
    {
        ...
        const float scale = juce::jmin (body.getWidth() / src.getWidth(), body.getHeight() / src.getHeight());

        auto t = juce::AffineTransform::translation (-src.getWidth() * 0.5f, -src.getHeight() * 0.5f).scaled (scale);
    ...
    const auto dot =
        juce::Rectangle<float> (dotRadius * 2.0f, dotRadius * 2.0f)
            .withCentre (centre.translated (dotDistance * std::sin (angle), -dotDistance * std::cos (angle)));
...
void PedalLookAndFeel::drawLinearSlider (juce::Graphics& g,
                                         int x,
                                         int y,
                                         int width,
                                         int height,
                                         ...
                                         juce::Slider& slider)
{
    ...
    const auto nodeFill = nodeOverridden ? slider.findColour (juce::Slider::trackColourId) : stemColour;
    const auto nodeBorder = nodeOverridden ? juce::Colours::black : stemColour.brighter (0.4f);
...
void PedalLookAndFeel::drawButtonBackground (
...
void PedalLookAndFeel::drawCornerResizer (juce::Graphics& g, int w, int h, bool isMouseOver, bool isMouseDragging)
{
