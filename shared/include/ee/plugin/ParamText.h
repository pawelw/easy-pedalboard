...
/** Parameter value formatters shared by more than one pedal.
    ...
    - `hzToText` rounds to a whole number in BitBit Wah but keeps a decimal place in
      BitBit Chorus and BitBit Phase. The decimal form is the one below; Wah keeps its
      own.
    - `decibelsToText` prints "3.0 dB" in BitBit Overdrive and a bare signed integer
      in BitBit EQ, whose fader row is too narrow for a unit.
    - BitBit Reverb's `hertzToText`, BitBit EQ's cut readouts and BitBit Wah's
...
/** "0.75 Hz" below 1 Hz, "3.2 Hz" above it - an LFO rate slow enough that the
    decimal matters. BitBit Wah's rates are fast enough that it rounds instead. */
